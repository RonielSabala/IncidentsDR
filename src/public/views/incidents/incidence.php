...
use App\Utils\{Entities\UserUtils, PrintUtils};
...
<div class="incidence-container">
    <div class="card-min mb-4">
            ...
            <div class="incidence-card">
                <div class="incidence-main">
                    <div class="incidence-top">
                        <span
                            class="kicker"><?= empty($labels) ? 'NOTICIA' : strtoupper($labels[0]['label_name']); ?></span>
                    ...
                    <h2 id="incidence-title" class="title">
                        <?= $title; ?></h2>
                    <p class="incidence-meta" aria-label="Metadatos de la noticia">
                        <time
                            datetime="<?= date('c', strtotime($created)); ?>">Fecha
                            de publicación:
                            <strong><?= $created; ?></strong></time>
                        <span class="dot"></span>
                        <span>Ocurrencia:
                            <strong><?= $occurrence; ?></strong></span>
                        <span class="dot"></span>
                        <span>Publicado por:
                            <strong><?= $reporterName; ?></strong></span>
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="card-section pb-4">
            ...
            <div class="section-description">
                ...
                <div class="sr-only" aria-hidden="false">Enlaces en azul; cada enlace aparece en una línea separada.
                </div>
            </div>
            ...
            <div class="mt-1rem">
                ...
                <div class="info-grid">
                    <div class="info-row">
                        <div class="info-pill pill-deaths">Muertos: <span
                                class="info-pill-value"><?= $deaths; ?></span>
                        </div>
                        <div class="info-pill pill-injured">Heridos: <span
                                class="info-pill-value"><?= $injured; ?></span>
                        </div>
                        <div class="info-pill pill-losses">Pérdidas: <span class="info-pill-value">RD$
                                <?= $losses; ?></span></div>
                    </div>
                </div>
            </div>
                ...
                <div class="no-photos-card">
                    ...
                    <div class="no-photos-icon" aria-hidden="true">
                        <svg width="96" height="72" viewBox="0 0 96 72" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <rect x="1" y="1" width="94" height="70" rx="10" stroke="currentColor" stroke-opacity="0.12"
                                stroke-width="2" fill="none" />
                            <g transform="translate(14,12)" stroke="currentColor" stroke-width="2"
                                stroke-linecap="round" stroke-linejoin="round">
                                <rect x="0" y="6" width="68" height="44" rx="6" fill="none" stroke-opacity="0.14">
                                </rect>
                            ...
                            <path d="M18 56 L78 16" stroke="currentColor" stroke-width="3" stroke-linecap="round"
                                stroke-opacity="0.08" />
                        </svg>
                    </div>
            ...
            <div class="text-end">
                <a id="btnGoToCorrectionPage" class="btn btn-outline-success btn-sm"
    ...
    <!-- Comentarios -->
    <div>
        <h5 class="comments-heading">Comentarios <span class="small-muted">·
                <?= count($comments); ?></span></h5>

        <!-- Añadir comentario -->
        <div class="card card-section pb-3 mb-3">
            <form method="post"
                action="incidence.php?id=<?= $incidenceId; ?>&action=POST"
                class="form-comment" autocomplete="off">
                <div class="comment-form-row">
                    <div class="comment-avatar avatar-color-<?= $current_user_color_idx; ?>"
                        aria-hidden="true">
                        <?= $current_user_initial; ?>
                    </div>
                    <div class="comment-form-body">
                        <textarea name="comment_text" class="form-control" placeholder="Añade un comentario..."
                            required></textarea>
        ...
        <div class="card card-section pb-4">
            ...
            <?php foreach ($comments as $c) {
                ...
                ?>
            <div
                class="comment-row d-flex align-items-start <?= $commentRow; ?>">
                <!-- Avatar: clase de color en vez de style -->
                <div class="comment-avatar <?= $commentAvatar; ?> avatar-color-<?= $acolor_idx; ?>"
                ...
                <div class="comment-body">
                    <div class="d-flex align-items-center comment-author-row">
                        <div>
                            <div class="comment-author">
                                ...
                                <?php if (!empty($badge)) { ?>
                                <span
                                    class="<?= $badge[0]; ?>"><?= $badge[1]; ?></span>
                                <?php } ?>
                            </div>
                            <div class="comment-meta"><?= $cdate; ?>
                            </div>
                        </div>
                        ...
                        <div class="ms-auto d-flex align-items-center">
                            ...
                            <form method="post"
                                action="incidence.php?id=<?= $incidenceId; ?>&action=DELETE"
                    ...
                    <div class="comment-text">
                        <?= PrintUtils::getPrintableText($ctext); ?>
                    </div>
                </div>
            </div>
            <?php } ?>
